...
import { Search, Trash2, Plus, Loader2, ChevronDown, ChevronUp, ExternalLink, Bot, ArrowLeft } from "lucide-react";
...
export function StrategistView() {
  ...
  const showAnalysisPanel = !showNewAnalysis && (analysisInProgress || isAnalyzing || !!selectedAnalysis);

  const mobileShowContent =
    leftTab === "analyses" ? showAnalysisPanel || showNewAnalysis : !!selectedResearch;
  ...
  return (
    <PageContainer className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6 shrink-0">
        <h1 className="text-xl font-semibold tracking-[-0.02em]">Strategist</h1>
        {leftTab === "analyses" && !mobileShowContent && (
          <Button
            variant="ghost"
            size="sm"
        ...
        {leftTab === "analyses" && mobileShowContent && (
          <Button
            variant="ghost"
            size="sm"
            className="md:hidden h-8 gap-1.5 text-xs text-muted-foreground"
            onClick={handleNewAnalysis}
          >
            <Plus className="size-3.5" />
            New
      ...
      {/* Main layout */}
      <div className="flex flex-1 min-h-0 gap-0 md:gap-8">

        {/* Sidebar */}
        <nav className={`flex-col gap-0.5 w-full md:w-44 shrink-0 ${mobileShowContent ? "hidden md:flex" : "flex"}`}>
            ...
            {leftTab === "analyses" ? (
              <>
                {analyses.length === 0 && !analysisInProgress && (
                  <p className="text-xs text-muted-foreground px-3 py-4 text-center">No analyses yet</p>
                )}
                {analysisInProgress && (
                  <div className="flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground rounded-md">
                ...
                {analyses.map((a) => (
                  <div
                    key={a.id}
                    className={`group flex items-start justify-between px-3 py-2 rounded-md cursor-pointer transition-colors text-sm ${
                      selectedId === a.id && !showNewAnalysis
                        ? "bg-muted font-medium text-foreground"
                        : "text-muted-foreground [@media(hover:hover)]:hover:text-foreground [@media(hover:hover)]:hover:bg-muted/60"
                    }`}
                    onClick={() => { setShowNewAnalysis(false); selectAnalysis(a.id); }}
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className="truncate text-xs">
                          {a.csvSummary.dateRange.from} – {a.csvSummary.dateRange.to}
                        </span>
                        {a.autoGenerated && <Bot className="size-3 shrink-0 text-muted-foreground" />}
                      </div>
              ...
              <>
                {researchNotes.length === 0 && (
                  <p className="text-xs text-muted-foreground px-3 py-4 text-center">No research notes yet</p>
                )}
                {researchNotes.map((n) => (
                  <div
                    key={n.id}
                  ...
                  >
                    <p className="truncate text-xs">{n.topic}</p>
        ...
        {/* Content */}
        <div className={`flex-1 min-w-0 overflow-y-auto flex-col ${mobileShowContent ? "flex" : "hidden md:flex"}`}>
          {/* Mobile back */}
          {leftTab === "analyses" && (
            <button
          ...
          {leftTab === "research" ? (
                      ...
                      <div key={i} className="rounded-md border p-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <a
                            ...
                            className="shrink-0 text-muted-foreground [@media(hover:hover)]:hover:text-foreground"
                          >
                            <ExternalLink className="size-3.5" />
                          </a>
                        </div>
                        {s.snippet && (
                          <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{s.snippet}</p>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="max-w-2xl w-full pb-8">
                  ...
                  <div className="border-t px-3 py-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Input className="h-8 text-sm" placeholder="Ruslan Buchak" value={profile.name} onChange={(e) => updateProfile({ name: e.target.value })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Username</Label>
                        <Input className="h-8 text-sm" placeholder="razRBCHK" value={profile.username} onChange={(e) => updateProfile({ username: e.target.value })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Followers</Label>
                        <Input className="h-8 text-sm" placeholder="1200" value={profile.followers} onChange={(e) => updateProfile({ followers: e.target.value })} />
                      </div>
                      <div className="space-y-1">
                        <Label className="text-xs">Following</Label>
                        <Input className="h-8 text-sm" placeholder="350" value={profile.following} onChange={(e) => updateProfile({ following: e.target.value })} />
                      </div>
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">Bio</Label>
                      <Textarea className="text-sm resize-none" rows={2} placeholder="About you..." value={profile.bio} onChange={(e) => updateProfile({ bio: e.target.value })} />
                    </div>
                  </div>
                )}
              </div>
              ...
              {analysisError && <p className="text-sm text-destructive mt-2">Error: {analysisError}</p>}

              {csvSummary && !csvError && (
                <div className="mt-3 rounded-md border bg-muted/30 p-3 text-sm">
                  <p className="font-medium">Preview</p>
                  <p className="text-muted-foreground">
                    {csvSummary.totalPosts} posts · {csvSummary.dateRange.from} to {csvSummary.dateRange.to} · avg {csvSummary.avgImpressions} impressions
                  </p>
                </div>
              )}
              ...
              <Button
                ...
                onClick={() => { setShowNewAnalysis(false); runAnalysis(); }}
              >
                {analysisInProgress ? (
              ...
              {(analysisInProgress || searchQueries.length > 0) && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {searchQueries.map((q, i) => (
                    <div key={i} className="flex items-center gap-1 rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">
